...
CUSUM_K = 0.0125  # calibrated on real DS-1 delta-pH noise floor
CUSUM_H = 0.08    # calibrated to recover non-zero events with near-zero FP proxy
...
# Phase 3 reward-alignment lock:
# compliance-first objective needs clear ordering by compliance level.
W_COMP, W_DEV, W_DOSE, W_OVER, W_ESC = 3.0, -1.0, -0.3, -0.5, -0.1
# Phase 4 fix1 (principled): when already compliant, non-null dosing should be discouraged.
# This term encodes "if in-band, prefer stop dosing" independent of PPO outcomes.
W_INBAND_DOSE = -1.0
W_UNC = -0.5
...
# Noise model (Phase 1d): separate process vs observation pH noise.
# Process noise models unresolved plant disturbances in true state evolution.
SIGMA_PH = 0.001
# Observation noise models sensor uncertainty (ISO 10523-scale).
SIGMA_PH_OBS = 0.02
...
def reconstruct_actions(ds1: pd.DataFrame, A_T: float, C_T: float) -> np.ndarray:
    ...
    # DISABLED (Phase 0 reconciliation): CUSUM-based action reconstruction belongs to
    # the prior real-data framing and is not part of the simulation-only Route A study.
    #
    # Legacy implementation retained below as comments for traceability:
    ...
    # return acts
    raise RuntimeError(
        "Disabled legacy path: reconstruct_actions (CUSUM + inverse assignment) is not used in "
        "the simulation-only Route A study."
    )
...
class WastewaterMDP:
    """Table 8 observations with pure first-principles physics transitions."""

    def __init__(
        self,
        lstm: Optional[LSTMSurrogate],
        A_T: float,
        C_T: float,
        device: torch.device,
        physics_warm: int,
        ...
        mm: MinMaxScaler,
        mm_cols: List[str],
        dph_scaler: Optional[StandardScaler],
        sigma_ph: float,
        sigma_model: float,
        sigma_cond: float,
        augment_observations: bool = False,
        sigma_ph_obs: float = SIGMA_PH_OBS,
        ...
        self.sigma_process_ph = sigma_ph
        self.sigma_obs_ph = sigma_ph_obs
        self.sigma_model = sigma_model
        self.sigma_cond = sigma_cond
        self.augment_observations = augment_observations
        self.enable_curriculum_masking = enable_curriculum_masking
        self.enable_escalation_penalty = enable_escalation_penalty
        self.global_step = 0
    ...
    def _apply_obs_noise(self, obs: np.ndarray, rng: np.random.Generator) -> np.ndarray:
        """Apply sensor noise to observed pH only (not true transition state)."""
        o = obs.copy()
        i_ph = self.mm_cols.index("pH_raw")
        span_ph = float(self.mm.data_max_[i_ph] - self.mm.data_min_[i_ph] + 1e-9)
        o[0] += float(rng.normal(0, self.sigma_obs_ph / span_ph))

        # DISABLED (Phase 1d noise-model correction): legacy observation augmentation
        # used a single pH sigma and extra cond noise toggle. Route A keeps pH sensor
        # noise only in observation space.
        #
    ...
    def step(self, action: int, rng: np.random.Generator):
        ...
        # Route A (simulation-only): always use first-principles titration transition.
        ph2_clean = f_titration(self.ph, action, self.A_T, self.C_T)

        # DISABLED (Phase 1b refactor): legacy hybrid physics->LSTM transition path.
        # Kept for traceability only; not executed in simulation-only Route A.
        #
        ...
        ph2 = ph2_clean + float(rng.normal(0, self.sigma_process_ph))

        # DISABLED (Phase 1d noise-model correction): legacy single-sigma transition noise.
        ...
        in_comp = bool(PH_LO <= self.ph <= PH_HI)
        Rc = 1.0 if in_comp else -1.0
        # DISABLED (Phase 3 reward-alignment): legacy zero-floor compliance term.
        # Kept for traceability only.
        #
        ...
        if not self.enable_escalation_penalty:
            Re = 0.0
        Rinband = float(in_comp and (action != 0))
        r = W_COMP * Rc + W_DEV * Rd + W_DOSE * Rdo + W_OVER * Ro + W_ESC * Re + W_INBAND_DOSE * Rinband

        # DISABLED (Phase 1b refactor): MC-dropout uncertainty penalty from surrogate model.
        # Legacy code retained for traceability only.
        #
        ...
        # Phase 3 reward-fix: train PPO on raw reward directly.
        # Rationale: running z-score normalization inverted policy ordering
        # (high-compliance policies scored lower than null on normalized signal).
        train_r = float(r)

        # DISABLED (Phase 3 reward-fix): running reward normalization layer.
        # Kept for traceability only.
        #
        # train_r = self.reward_norm.norm(r)
        ...
        return obs, float(train_r), bool(done), {"raw_reward": r, "dyna_x": xa_np, "dyna_dph": float(ph2_clean - ph0)}

    def action_mask(self, rng: np.random.Generator) -> np.ndarray:
        m = np.ones(11, dtype=np.float32)
        if not self.enable_curriculum_masking:
            return m
...
def run_full_pipeline(
    ...
    # DISABLED (Phase 0 reconciliation): this function wires the legacy real-data ->
    # CUSUM labels -> LSTM surrogate training pipeline and should not be used in the
    # simulation-only Route A framing.
    #
    # The full legacy implementation is intentionally retained below for traceability.
    raise RuntimeError(
        "Disabled legacy path: run_full_pipeline uses real-data reconstruction + LSTM surrogate "
        "training. Use simulator-only study entrypoints for Route A."
    )
