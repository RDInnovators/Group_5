...
def methodology_first_pass_small(
    ...
    """
    # DISABLED (Phase 0 reconciliation): this helper depends on legacy
    # real-data/CUSUM/LSTM-surrogate pipeline and includes a real-data Tier-3 note.
    # It is out of scope for the simulation-only Route A study.
    #
    # Legacy implementation retained below for traceability.
    raise RuntimeError(
        "Disabled legacy path: methodology_first_pass_small belongs to the prior "
        "real-data + surrogate framing and is not valid for simulation-only Route A."
    )
